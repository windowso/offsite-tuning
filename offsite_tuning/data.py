...
def get_raw_datasets(args):
    ...
    if args.dataset_name is not None:
        ...
        raw_datasets = load_dataset(
            dataset_name, dataset_config_name) # {'train':{'text':['text1','text2',...],...}, 'validation':{'text':['text1','text2',...],...}, 'test':{'text':['text1','text2',...],...]}}
...
def get_tokenized_datasets(raw_datasets, args, accelerator, tokenizer, lm_type='clm'):
    ...
    # lm: language model
    # lm_type: clm: causal language model(e.g.GPT), mlm: masked language model(e.g.BERT)
...
def get_lm_datasets(tokenized_datasets, args, accelerator, tokenizer, lm_type='clm'):
    ...
    # for each of those groups of 1,000 texts. You can adjust that batch_size here but a higher value might be slower to preprocess.
    #
...
def process_text2text_datasets(raw_datasets, args, tokenizer, accelerator):
    ...
    def tokenize_function(examples):
        context = task.get_context(examples) # list of str
        target = task.get_target(examples) # list of str
        ...
        out['input_ids'] = [i1 + i2 for i1,
                            i2 in zip(context['input_ids'], target['input_ids'])] # list of list
        # for list, + means concatenation
    ...
    max_length = -1
    for v in tokenized_datasets.values(): # v is a list of dict
        for x in v: # x is a dict
            max_length = max(max_length, len(x['input_ids']))

    # pad to the multiple of 8
    max_length = (max_length // 8 + 1) * 8 # // is floor division

    block_size = _get_block_size(args, tokenizer) # usually 1024 in this code
    max_length = min(max_length, block_size)

    def pad_function(examples):
        examples["input_ids"] = [i + [tokenizer.pad_token_id] *
                                 (max_length - len(i)) for i in examples["input_ids"]] # i is a list
        ...
        # if len(i) > max_length, list * (max_length  - len(i)) = [], so we should truncate.
